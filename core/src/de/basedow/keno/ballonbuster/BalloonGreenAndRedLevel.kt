...
class BalloonGreenAndRedLevel(game: Game) : BaseScreen(game) {
    ...
    override fun update(delta: Float) {
        ...
        if (spawnTimer > spawnInterval) {
            ...
            val ballon = Balloon()
            val green = MathUtils.randomBoolean()
            ballon.color = if (green) Color.GREEN  else Color.RED
            ballon.addListener(object : InputListener() {
                override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
                    if (green) popped++ else popped--
